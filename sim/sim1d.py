...
def sim_run(options, KalmanFilter):
    ...
    est_data_t  = []
    v_est_data  = []
    x_est_data  = [] # X error : estimated - real
    X_estimated = [] 
    X_measured  = []
    ...
    for t0 in t:
        ...
        if t0%1.0 == 0.0:
            ...
            # estimate X
            X_measured += [state_with_noise]
            X_estimated += [kalman_filter.predict(t0)]
            x_est_data += [kalman_filter.predict(t0) - state[-1]] # X error : estimated - real
