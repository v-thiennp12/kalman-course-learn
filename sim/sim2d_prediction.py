...
def sim_run(options, KalmanFilter, light_state):
    ...
    # Main plot info.
    car,        = ax.plot([], [], 'r-', linewidth = 5)
    light,      = ax.plot([94,94], [4,2] , 'g-', linewidth = 3)
    est_light,  = ax.plot([], [], 'g--', linewidth=1)
    est,        = ax.plot([], [], 'bs', markersize=7.5, fillstyle='full',   linewidth=5)
    meas,       = ax.plot([], [], 'gs', markersize=30,  fillstyle='none',   linewidth=5)
    ...
    # Animation.
    car_ani = animation.FuncAnimation(fig, update_plot, frames=range(1,len(t), 1),
